  ...
  module Actions
    class NoticeToBicAction < Action
      def self.run(params)
        ...
        bot_id = params[2]
        group_ids = ["9d06673d-03f1-4d27-85df-7ac0eff2c375"] # id of group [Staging] Bein Community/ App Notice
        UI.message "content: #{content}"
        UI.message "mention_user_ids: #{mention_user_ids}"
        UI.message "group_ids: #{group_ids}"

        response = Faraday.post "https://api.beincomm.app/v1/feed/posts/bot" do |request|
