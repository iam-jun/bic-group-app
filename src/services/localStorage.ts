...
const removeAllData = async (exception_keys: Array<string>): Promise<boolean> => {
  try {
    const all_keys = await AsyncStorage.getAllKeys();
    if (all_keys) {
      // get key of app
      const app_keys = all_keys.filter((value) => {
        const exception_key = value.replace(
          STORE_BEIN, '',
        );
...
const setLanguage = async (language: string): Promise<void> => storeData(
  language, 'language',
);
