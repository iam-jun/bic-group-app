  ...
  return new Promise((resolve, reject) => {
    https.get(url, function (res) {
      if (res.statusCode !== 200)
        reject(`Bad response code: ${res.statusCode}`);
...
download(emojiSrc)
  ...
  require('fs').writeFileSync('visible.txt', text, 'utf8');*/
    ...
    fs.writeFileSync(
      join(__dirname, '../lib/data/full.json'),
      JSON.stringify(emojies, null, 2),
      'utf8',
    );
    ...
    fs.writeFileSync(
      join(__dirname, '../lib/data/light.json'),
      JSON.stringify(emoji_light, null, 2),
      'utf8',
    );
  })
