...
const Landing = () => {
    ...
    const title = groupName ? t('auth:text_landing_title_group').replace('{0}', groupName) : t('auth:text_landing_title');
    const desc = groupName ? t('auth:text_landing_desc_group').replace('{0}', groupName) : '';

    return (
        <ScreenWrapper isFullView style={styles.container}>
            {!!logo && <Image resizeMode="contain" style={styles.logo} source={{ uri: logo }} />}
            ...
            <PrimaryButton max title={t('auth:btn_landing_start')} style={styles.button} onPress={() => navigation.navigate(authStack.login)}/>
        </ScreenWrapper>
    );
}
