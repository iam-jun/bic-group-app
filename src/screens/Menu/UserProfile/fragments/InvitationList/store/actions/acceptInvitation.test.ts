...
describe('acceptInvitation in user profile', () => {
  afterEach(() => {
  ...
  it('should do nothing if invitationId is undefined', () => {
    ...
    act(() => {
      result.current.actions.acceptInvitation(undefined, 1);
    });
  ...
  it('should do nothing if requestingsAccept invitationId is true', () => {
    ...
    const { result } = renderHook(() => useMyInvitationsStore((state) => state));
    act(() => {
      result.current.actions.acceptInvitation(invitationId, 1);
    });
  ...
  it('should do nothing if can not find group index in groupedInvitations', () => {
    ...
    const { result } = renderHook(() => useMyInvitationsStore((state) => state));
    act(() => {
      result.current.actions.acceptInvitation(invitationId, 3);
    });
  ...
  it('should decline invition success', () => {
    ...
    expect(result.current.requestingsAccept?.[invitationId]).toBeTruthy();
    ...
    expect(result.current.requestingsAccept?.[invitationId]).toBe(undefined);
  ...
  it('should decline invition failed with is already declined', () => {
    ...
    expect(result.current.requestingsAccept?.[invitationId]).toBe(true);
