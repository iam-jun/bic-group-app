...
const SearchDiscoverCommunity = ({
  initSearch,
  ...props
}: SearchDiscoverCommunityProps) => {
  const theme: ExtendedTheme = useTheme();
  const [searchText, setSearchText] = useState(initSearch || '');
  const styles = createStyles();

  const { hasNextPage, actions, reset } = useDiscoverCommunitiesSearchStore();

  const getCommunitySearch = (searchText: string) => {
    actions.getDiscoverCommunitiesSearch({ key: searchText });
  };
...
export default SearchDiscoverCommunity;
