...
function* markAsRead({ payload }: {payload: any; type: string}): any {
  try {
    ...
    const tabUnreadData: any[] = cloneDeep(
      yield select((state) => get(state, notificationSelector.notificationByType('tabUnread'))),
    ) || [];
    let newData = [];
    ...
    console.error('\x1b[33m', 'notification markAsRead error', err, '\x1b[0m');
  }
}
